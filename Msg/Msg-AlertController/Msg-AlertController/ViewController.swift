...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let alert2 = UIAlertController(title: "네트워크 오류", message: "네트워크 연결을 확인하세요.", preferredStyle: .alert)
    ...
    @IBAction func login(_ sender: Any) {
        ...
        let message = "사용자의 Apple ID 97chos@naver.com의 암호를 입력하세요"
    ...
    @IBAction func auth(_ sender: Any) {
        ...
        let cancel = UIAlertAction(title: "취소", style: .cancel)
        let ok = UIAlertAction(title: "확인", style: .default){_ in let loginId = alert.textFields?[0].text
        ...
        alert.addTextField(configurationHandler: { tf in tf.placeholder = "아이디"
            tf.isSecureTextEntry = false
        })
        
        alert.addTextField(configurationHandler: {tf in tf.placeholder = "비밀번호"
            tf.isSecureTextEntry = true
        })
