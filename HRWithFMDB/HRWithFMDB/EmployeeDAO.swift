...
class EmployeeDAO {
    // FMDatavase 객체 생성 및 초기화
    lazy var fmdb: FMDatabase = {
        // 1. 파일 매니저 객체 생성
        let fileMgr = FileManager.default

        // 2. 샌드박스 내 문서 디렉터리 경로에서 데이터베이스 파일 경로를 읽어옴
        ...
        // 4. 준비된 데이터베이스 파일을 바탄으로 FMDatabase 객체 생성
    ...
    func get(empCd: Int) -> EmployeeVO? {
        // 1. 질의 실행
        let sql = """
            SELECT emp_cd, emp_name, join_date, state_cd, department.department_title
            FROM employee
            JOIN department ON department.depart_cd = employee.depart_cd
            WHERE emp_cd = ?
        """
        ...
        if let _rs = rs {
            ...
            record.departTitle = _rs.string(forColumn: "depar_title")!
